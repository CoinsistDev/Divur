...
export const getCannedRepliesTitleEvent = async (departmentId, text) => {
  const cannedReplies = await DepartmentService.getGlassixCannedReplies(departmentId, true);
  const cannedRepliesText = [];
...
const handleEvent = async (events) => {
  try {
    ...
    let data = events.map(async (event) => ({
      Date: event.dateTime,
      From: event.changes[0].message.from,
      To: event.changes[0].message.to,
      Text: event.changes[0].message.text.substring(0, 500),
      ...
      isBlackList: false,
      cannedRepliesTitle: event.changes[0].message.status === 'Accepted' ? await getCannedRepliesTitleEvent(departmentId, event.changes[0].message.text) : '',
      ProviderMessageId: event.changes[0].message.providerMessageId,
    }));

    data = await Promise.all(data);

    if (sentWhatsAppFaildCount) DepartmentService.updateCountMessage({ departmentId, count: sentWhatsAppFaildCount, protocolType: 'WhatsApp' }, 'Incrementing');
