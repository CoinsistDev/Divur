...
export default observer(function DistributionForm() {
  ...
  const [distributionValues, setDistributionValues] =
    useState<DistributionFormValues>(new DistributionFormValues());
  ...
  useEffect(() => {
    if (id) {
      loadCurrentDepartment(id).then((department) => {
        ...
        const smsDisabled = department
          ? department.remainingSMSMessages === 0
            ? true
            : false
          : true;
        setMessagingOptions([
    ...
    <Grid>
      ...
      <Grid.Column computer={8}>
        <Container
          fluid
          textAlign="right"
          style={{ marginTop: "40px", direction: "rtl" }}
        >
          <Header as="h2" style={{ paddingBottom: "20px" }}>
            ...
            <Header.Content>
              הפצה
              <Header.Subheader>
                שלחו הודעות וואטספ ללקוחות שלכם
              </Header.Subheader>
            </Header.Content>
            <Divider />
          </Header>
              ...
              >
                ...
                <Dropdown
                  ...
                  onOpen={() =>
                    ...
                    loadCannedReplies(currentDepartment?.id)
                  }
                  onChange={(
                    e: SyntheticEvent<HTMLElement>,
                    { name, value }: any
                          ...
                          <Grid.Column computer={4}>
                            ...
                            <Button
                              ...
                              onClick={() => {
                                values.parameters = [];
                                var pattern = /[^{{}}]+(?=\})/g;
                                var paramsToAdd = values.message
                                  .match(pattern)
                                  ?.filter((v, i, a) => a.indexOf(v) === i);

                                if (
                                  (paramsToAdd?.length || undefined) &&
                                  paramsToAdd!.length > 0
                                ) {
                        ...
                        {values.parameters.map((parameter, i) => (
                          <Container key={`pContainer${i}`}>
                            <sForm.Group>
                              <CustomTextInput
                                width={4}
                                name={`parameters.${i}.messageParameter`}
                                placeholder="שם הפרמטר בהודעה"
                              />
                              <CustomTextInput
                                width={4}
                                name={`parameters.${i}.fileParameter`}
                ...
                {values.isTimed && (
                  <CustomDateInput
                    placeholderText="מועד ההפצה"
                    name="scheduleDate"
                    showTimeSelect
                    timeCaption="שעה"
                    dateFormat="תאריך yyyy/MM/dd בשעה HH:mm"
                  />
                )}
                ...
                <sForm.Field width={6}>
                  ...
                  <a
                    ...
                    href={`https://docs.google.com/spreadsheets/d/1X4w8ErfJBc49_8NZMOMNPHTD6WecQXSeYehf-3ZdeXk`}
                  >
                    קובץ לדוגמא
                  </a>
                  <Button
                  ...
                  >
                    {distributionValues.file.length === 0
                      ? `העלאת קובץ`
                      : distributionValues.file[0].name}
                  </Button>
                  <input
                    type="file"
                    id="file"
                ...
                <Button
                  ...
                  content="שלח"
                  type="submit"
                  disabled={isSubmitting || !isValid || !dirty || submitted}
                ...
                {submitted && (
                  <Message
                    ...
                    content="הפקודה התקבלה בהצלחה, ההודעות ישלחו בהקדם"
                  />
                )}
                {errors.length > 0 && (
