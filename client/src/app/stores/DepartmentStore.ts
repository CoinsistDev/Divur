...
export default class departmentStore {
      ...
      runInAction(() => {
        if (this.currentDepartment !== undefined) {
          this.currentDepartment.scheduledDistributionTasks.forEach((task) => {
            if (task.taskId === jobId) task.status = 3;
          });
        }
        this.loading = false;
      });
  ...
  getMessageStats = async (id: string, startDate: string, endDate: string) => {
    try {
      ...
      runInAction(() => {
        if (this.currentDepartment) this.currentDepartment!.totalNonTicketMessageDelivered = result.totalNonTicketMessageDelivered!;
  ...
  sendTaskReport = async (id: string, startDate: Date | null, endDate: Date | null, filterData: ScheduledTask[]) => {
    try {
      this.loading = true;
      await agent.Reports.scheduledDistribution(id, startDate, endDate, filterData);
      runInAction(() => (this.loading = false));
    } catch (err) {
      console.log(err);
      runInAction(() => (this.loading = false));
